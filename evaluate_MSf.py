...
def evaluate_MSf(ComputerPrecisions,Channels,CoulombFunctions_data,CoulombFunctions_poles, Dimensions,Logicals, 
...
#   if True:
        ...
        @tf.function        
        def FitMeasureTF(searchpars,datas,model):
            
            n_angle_integrals0 = n_angles                # so [n_angle_integrals0,n_totals0] for angle-integrals
            n_totals0 = n_angles + n_angle_integrals  
...
#         chisqF,Grads = FitMeasureTF(searchpars,datas,model)  # A_tF,Grads,  Tp_mat, XSp_mat,XSp_tot via globals
        tot_chisq, tot_Grads = 0.0, tf.zeros_like(searchpars)
        for n in range(npal):
            this_part = split_data_model(n)
            ...
            per_replica_chisq = strategy.run( FitMeasureTF, args=(searchpars,this_part[0],this_part[1]) )
            chisq_n = strategy.reduce( tf.distribute.ReduceOp.SUM,  per_replica_chisq[0], axis = None)
            Grads_n = strategy.reduce( tf.distribute.ReduceOp.SUM,  per_replica_chisq[1], axis = 0)
            tot_chisq += chisq_n
            tot_Grads += Grads_n 
        chisqF,Grads = tot_chisq, tot_Grads 
