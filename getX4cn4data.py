        ...
        for Out in more+channels:
            ...
            if nat is not None:  
                if t==nat:    # replace t by nat-t as isotopically dominant
                    natT = t.split('-')[0]
                    tn = natT + '-0'
                    ext = '-nat'+natT if nat and natT!='' is not None else ''
                    print('Replace',t,'by',tn,' to file ext',ext)
                    t = tn
            
            for obs in allObs:
                print("Search X4 for ",t+'('+reaction+'),',obs)
...
#                         print('Err @',dData_index,dData_index1,dData_index2,'so % are',dataErr_rel,dataErr_rel1,dataErr_rel)
                        ...
                        for stuff in ds[d].data:
                            if level_index is not None and obs in ['DA','CS']:
                                ...
                                tex = nucname + ('_e%s' % int(levelnum) if levelnum > 0 else '')
                                try:
                                ...
                                tex = nucname + ('_e%s' % int(levelnum_g) if levelnum_g > 0 else '')
                                try:
                                    level = pops[tex].energy[0].pqu('MeV').value
                                except:
                                ...
                                Q = stuff[Q_index]
                                if Qgs is None:
                                    print("Qgs IS NOT KNOWN: SKIP DATA!!")
                                    continue
                                level = Qgs - Q
                                levels.add(level)
                            else:
                                level = 0.
#                             try:
#                                 if Eresid is not None and abs(Eresid-level)>Eres: continue
#                             except:
#                                 print('Eresid,level,Eres=',Eresid,level,Eres)
                            level_found = level
...
#                             print('dData2:',dData2,dataErr_abs2,Data,dataErr_rel2)
                            ...
                            #print E,Ang,Data,dE,dAng,dData
#                             Eeval = E
#                             if Eeval > min(emax,EnergyMaxLimit): continue
#                             if Eeval < min(emin,EnergyMinLimit): continue
